...
cat("ipwvax1 \n")
ipwvax1 <- parglm(
  formula = update(vax1 ~ 1, formula_demog) %>% update(formula_comorbs) %>% update(formula_secular_region) %>% update(formula_timedependent),
...
cat("ipwvax1_fxd \n")
ipwvax1_fxd <- parglm(
  formula = update(vax1 ~ 1, formula_demog) %>% update(formula_comorbs) %>% update(formula_secular_region),
  data = data_pt_atriskvax1,
  family=binomial,
  control = parglmparams,
  na.action = "na.fail",

...
# # # # # # # # # # # # # # # # # # # # #
# This script:
# combines outputs from the `report_stcox.R` scripts across different outcomes and brands
# # # # # # # # # # # # # # # # # # # # #
...
estimates_formatted <- estimates %>%
  transmute(
    ...
    HR =scales::label_number(accuracy = .01, trim=TRUE)(hr),
    HR_CI = paste0("(", scales::label_number(accuracy = .01, trim=TRUE)(hr.ll), "-", scales::label_number(accuracy = .01, trim=TRUE)(hr.ul), ")"),
...
estimates_formatted_wide <- estimates_formatted %>%
  select(outcome_descr, brand_descr, model, term, HR_ECI, VE_ECI) %>%
  pivot_wider(
    id_cols=c(outcome_descr, brand_descr, term),
    names_from = model,
    values_from = c(HR_ECI, VE_ECI),
    names_glue = "{model}_{.value}"
  )

write_csv(estimates, path = here("output", cohort, strata_var, recent_postestperiod, "combined", glue("stcox_estimates.csv")))
write_csv(estimates_formatted, path = here("output", cohort, strata_var, recent_postestperiod, "combined", glue("stcox_estimates_formatted.csv")))
write_csv(estimates_formatted_wide, path = here("output", cohort, strata_var, recent_postestperiod, "combined", glue("stcox_estimates_formatted_wide.csv")))
...
if(strata_var=="all"){
  for(deathtype in deathtypes){
    ...
    msmmod_effect <-
      ...
      geom_point(aes(y=hr, x=term_midpoint), position = position_dodge(width = 1.5), size=0.8)+
      geom_linerange(aes(ymin=hr.ll, ymax=hr.ul, x=term_midpoint), position = position_dodge(width = 1.5))+
      facet_grid(rows=vars(outcome_descr), cols=vars(brand_descr), switch="y")+
      scale_y_log10(
        breaks = c(0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1, 2, 5),
        limits = c(0.05, max(c(1, msmmod_effect_data_plot$hr.ul))),
        oob = scales::oob_keep,
        sec.axis = sec_axis(
          ~(1-.),
          name="Effectiveness",
    ...
    ## save plot
    ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot_{deathtype}.svg")), msmmod_effect, width=20, height=20, units="cm")
    ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot_{deathtype}.png")), msmmod_effect, width=20, height=20, units="cm")
    ...
    msmmod_effect_free <-
      ...
      geom_point(aes(y=log(hr), x=term_midpoint), position = position_dodge(width = 1.5), size=0.8)+
      geom_linerange(aes(ymin=log(hr.ll), ymax=log(hr.ul), x=term_midpoint), position = position_dodge(width = 1.5))+
    ...
    ## save plot
    ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot_free_{deathtype}.svg")), msmmod_effect_free, width=20, height=20, units="cm")
    ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot_free_{deathtype}.png")), msmmod_effect_free, width=20, height=20, units="cm")
  }
...
if(strata_var!="all"){
  ...
  ## save plot
  ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot.svg")), msmmod_effect, width=20, height=20, units="cm")
  ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot.png")), msmmod_effect, width=20, height=20, units="cm")
  ...
  ## save plot
  ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot_free.svg")), msmmod_effect_free, width=20, height=20, units="cm")
  ggsave(filename=here("output", cohort, strata_var, recent_postestperiod, "combined", glue("VEstcox_plot_free.png")), msmmod_effect_free, width=20, height=20, units="cm")

...
data_tte <- read_rds(here::here("output", "modeldata", "data_over80s.rds")) # wide (one row per patient)
data_tte_cp <- read_rds(here::here("output", "modeldata", "data_cp_over80s.rds")) # counting-process (one row per patient per event)

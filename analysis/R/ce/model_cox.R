...
data_tte <- data_tte %>%
  mutate(
    ...
    tte_covidtest =tte(covid_vax_1_date, covid_test_1_date, lastfup_date, na.censor=FALSE),
    ind_covidtest = censor_indicator(covid_test_1_date, lastfup_date),

    tte_postest = tte(covid_vax_1_date, positive_test_1_date, lastfup_date, na.censor=FALSE),
    ind_postest = censor_indicator(positive_test_1_date, lastfup_date),

    tte_emergency = tte(covid_vax_1_date, emergency_1_date, lastfup_date, na.censor=FALSE),
    ind_emergency = censor_indicator(emergency_1_date, lastfup_date),

    tte_covidadmitted = tte(covid_vax_1_date, covidadmitted_1_date, lastfup_date, na.censor=FALSE),
    ind_covidadmitted = censor_indicator(covidadmitted_1_date, lastfup_date),

    tte_coviddeath = tte(covid_vax_1_date, coviddeath_date, lastfup_date, na.censor=FALSE),
    ind_coviddeath = censor_indicator(coviddeath_date, lastfup_date),

    tte_noncoviddeath = tte(covid_vax_1_date, noncoviddeath_date, lastfup_date, na.censor=FALSE),
    ind_noncoviddeath = censor_indicator(noncoviddeath_date, lastfup_date),

    tte_death = tte(covid_vax_1_date, death_date, lastfup_date, na.censor=FALSE),
...
for(stratum in strata){
  ...
  data_cox_sub_split <- data_cox_sub %>%
    survSplit(
    ...
    mutate(
      timesincevax = str_pad(tstart, 3, pad="0"),
  ...
  tt_week <- function(x, t, ...){
    day <- x + t
    paste0("week ", str_pad(floor(day/7), 2, pad = "0"))
  }
